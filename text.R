...
base::identical(dim(Abc7), dim(Kcra))
dim(Abc7)
dim(Kcra)
base::identical(class(Abc7), class(Kcra))
...
# {lubridate} nicely handles various formats and separators.
...
# Do you ever use NOAA or UCAR data? You need to know how to examine a NetCDF file.
 getwd()
 setwd("/Users/sarahtucker/Documents/GitHub/potpourri")
nc <- nc_open('/Users/sarahtucker/Documents/GitHub/potpourri/ROMSsurface.nc')# Hint: Style tip #8
...
lon <- v1$dim[[1]]$vals 
lon# dim[[1]] is lon, verified above with v1$dim[[1]] 
lat <-  v1$dim[[2]]$vals
lat# dim[[2]] is lat
